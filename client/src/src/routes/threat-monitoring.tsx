...
const SEED: Event[] = [
  ...
  },
  {
    ...
    t: "12:41:08",
    sev: "high",
    type: "Anomalous API rate spike",
    src: "URA-CLI-781 · 4× NIN lookups / 9s",
    action: "FLAGGED",
  },
  {
    id: 3,
    t: "12:39:51",
    sev: "med",
    type: "MoH adapter latency anomaly",
    src: "moh-adapter-3 · +28%",
    action: "ALLOWED",
  },
  {
    id: 4,
    t: "12:38:14",
    sev: "high",
    type: "Geo-impossible session",
    src: "Officer OFC-2026-0119 · Kampala → Nairobi 4m",
    action: "BLOCKED",
  },
  {
    id: 5,
    t: "12:36:02",
    sev: "low",
    type: "Cert auto-rotation",
    src: "MLHUD adapter · routine",
    action: "ALLOWED",
  },
];
...
function ThreatMonitoring() {
      ...
      const samples: Omit<Event, "id" | "t">[] = [
        {
          ...
          src: "Gateway · nonce reuse",
          action: "BLOCKED",
        },
        {
          sev: "critical",
          type: "SQL injection attempt",
          src: "POST /v1/identity · sanitized & dropped",
          action: "BLOCKED",
        },
        {
          sev: "med",
          type: "MFA challenge failed",
          src: "Officer OFC-2026-0120 · attempt 2/3",
          action: "FLAGGED",
        },
        {
          sev: "low",
          type: "Token rotation completed",
          src: "URA service account",
          action: "ALLOWED",
        },
        {
          sev: "high",
          type: "Suspicious cross-ministry probe",
          src: "Adapter health scan · gateway policy",
          action: "BLOCKED",
        },
      ];
      const p = samples[Math.floor(Math.random() * samples.length)];
      setEvents((prev) =>
        [{ id: Date.now(), t: new Date().toLocaleTimeString(), ...p }, ...prev].slice(0, 14),
      );
      setCounts((c) => ({
  ...
  const actColor = (a: Event["action"]) =>
    a === "BLOCKED" ? "text-destructive" : a === "FLAGGED" ? "text-warning" : "text-success";
    ...
    <InternalShell>
      <div className="max-w-[1600px] mx-auto px-4 py-6">
        <div className="mb-6">
          ...
          <h1 className="text-2xl lg:text-3xl font-display font-bold mt-1">
            Security Operations Center
          </h1>
          <p className="text-sm text-muted-foreground mt-1">
            Welcome back, <span className="text-foreground">{session?.title}</span>. Real-time view
            of authentication failures, anomalous API behaviour and gateway policy events.
          </p>
        </div>
        ...
        <div className="grid sm:grid-cols-3 gap-3 mb-6">
          <Stat
            icon={Ban}
            label="Blocked (24h)"
            value={counts.blocked24h.toLocaleString()}
            color="text-destructive"
          />
          <Stat
            icon={AlertTriangle}
            label="Flagged (24h)"
            value={counts.flagged24h.toString()}
            color="text-warning"
          />
          <Stat
            icon={ShieldCheck}
            label="MFA challenges failed"
            value={counts.mfaFails.toString()}
            color="text-primary"
          />
        </div>
            ...
            <div className="divide-y divide-border/60 font-mono text-xs max-h-[520px] overflow-auto">
              {events.map((e) => (
                <div key={e.id} className="px-4 py-2.5 flex items-center gap-3 hover:bg-accent/30">
                  ...
                  <span
                    className={`px-1.5 py-0.5 rounded border text-[10px] uppercase ${sevColor(e.sev)}`}
                  >
                    {e.sev}
                  </span>
                  <span className="flex-1 font-sans text-foreground">{e.type}</span>
                  <span className="text-muted-foreground hidden md:inline truncate max-w-[280px]">
                    {e.src}
                  </span>
