...
export const MINISTRIES: Ministry[] = [
  {
    ...
    code: "NIRA",
    name: "NIRA",
    full: "National Identification & Registration Authority",
    color: "oklch(0.7 0.18 255)",
    angle: 0,
    capabilities: [
  ...
  },
  {
    ...
    code: "MLHUD",
    name: "Lands",
    full: "Ministry of Lands, Housing & Urban Development",
    color: "oklch(0.65 0.15 35)",
    angle: 120,
    capabilities: ["Land ownership", "Title validation", "Parcel registry"],
    endpoints: [{ name: "Ownership lookup", method: "GET", path: "/v1/lands/owner/{nin}" }],
  },
  {
    id: "mowt",
    code: "MoWT",
    name: "Works & Transport",
    full: "Ministry of Works & Transport",
    color: "oklch(0.72 0.12 220)",
    angle: 160,
    capabilities: ["Driving permit", "Vehicle registration", "Road safety"],
    endpoints: [{ name: "Permit check", method: "GET", path: "/v1/transport/permit/{nin}" }],
  },
  {
    id: "police",
    code: "UPF",
    name: "Police / Justice",
    full: "Uganda Police Force & Judiciary",
    color: "oklch(0.62 0.2 25)",
    angle: 200,
    capabilities: ["Criminal clearance", "Case status", "Court records"],
    endpoints: [{ name: "Criminal record", method: "GET", path: "/v1/justice/clearance/{nin}" }],
  },
  {
    id: "moes",
    code: "MoES",
    name: "Education",
    full: "Ministry of Education & Sports",
    color: "oklch(0.7 0.15 195)",
    angle: 240,
    capabilities: ["Academic records", "Certificate validation", "Institution registry"],
    endpoints: [{ name: "Certificate verify", method: "POST", path: "/v1/education/verify" }],
  },
  {
    id: "immig",
    code: "DCIC",
    name: "Immigration",
    full: "Directorate of Citizenship & Immigration Control",
    color: "oklch(0.68 0.16 305)",
    angle: 280,
    capabilities: ["Passport status", "Visa records", "Border crossings"],
    endpoints: [{ name: "Passport check", method: "GET", path: "/v1/immigration/passport/{nin}" }],
  },
  {
    id: "ucc",
    code: "UCC",
    name: "UCC",
    full: "Uganda Communications Commission",
    color: "oklch(0.72 0.12 280)",
    angle: 320,
    capabilities: ["SIM registration", "Telecom records", "Spectrum licensing"],
    endpoints: [{ name: "SIM lookup", method: "GET", path: "/v1/ucc/sim/{nin}" }],
  },
];
...
export const MOCK_CITIZEN = {
  ...
  properties: [
    {
      title: "FRV 4521 / Block 244 Plot 18",
      location: "Kira, Wakiso",
      type: "Residential",
      size: "0.25 ha",
    },
    {
      title: "LRV 1187 / Block 88 Plot 4",
  ...
  health: [
    {
      facility: "Mulago National Referral",
      visit: "2025-08-12",
      note: "Routine check-up",
      flag: "Restricted · MoH only",
    },
  ],
  telecom: [
...
export const ROLES = [
  {
    ...
    desc: "Inter-agency super-access · NGDXH oversight",
    color: "gold",
    grants: ["nira", "ura", "moh", "lands", "mowt", "police", "moes", "immig", "ucc"],
    denies: [],
  },
  {
    id: "nira",
    title: "NIRA Officer",
    desc: "Identity, civil registration & biometrics",
    color: "primary",
    grants: ["nira"],
    denies: ["police", "moh"],
  },
  ...
  },
  {
    ...
    desc: "Permits, vehicles & licensing",
    color: "primary",
    grants: ["mowt", "nira", "police"],
    denies: ["moh"],
  },
  {
    id: "police",
    title: "Police Officer",
    desc: "Justice & law enforcement",
    color: "destructive",
    grants: ["police", "nira", "immig", "mowt"],
    denies: ["moh"],
  },
  {
    id: "immig",
    title: "Immigration Officer",
    desc: "Borders, passports & visas",
    color: "primary",
    grants: ["immig", "nira", "police"],
    denies: ["moh"],
  },
  {
    id: "admin",
    title: "System Auditor",
    desc: "Read-only governance & audit logs",
    color: "primary",
    grants: ["nira", "ura", "moh", "lands", "mowt", "police", "moes", "immig", "ucc"],
    denies: [],
  },
...
const GATEWAY_EMAIL_BY_ROLE = {
  agency: "gateway@ngdxh.gov.ug",
  nira: "gateway@nira.go.ug",
  ura: "gateway@ura.go.ug",
  moh: "gateway@moh.go.ug",
  lands: "gateway@mlhud.go.ug",
  mowt: "gateway@mowt.go.ug",
  police: "gateway@upf.go.ug",
  moes: "gateway@moes.go.ug",
  immig: "gateway@dcic.go.ug",
  admin: "audit@ngdxh.gov.ug",
  ucc: "gateway@ucc.go.ug",
} as const;

const GATEWAY_CODE_SUFFIX = {
  agency: "240194",
  nira: "251107",
  ura: "251208",
  moh: "251309",
...
export const GATEWAY_MAILBOXES: GatewayMailbox[] = ROLES.map((role) => ({
  role: role.id,
  email:
    GATEWAY_EMAIL_BY_ROLE[role.id as keyof typeof GATEWAY_EMAIL_BY_ROLE] ??
    `gateway@${role.id}.go.ug`,
  label: `${role.title} inbox`,
  code: GATEWAY_CODE_SUFFIX[role.id as keyof typeof GATEWAY_CODE_SUFFIX],
}));

export const GATEWAY_CODE_BY_ROLE = GATEWAY_MAILBOXES.reduce<Record<string, string>>(
  (acc, mailbox) => {
    acc[mailbox.role] = mailbox.code;
    return acc;
  },
  {},
);
...
/**
 * Demo credentials. For showcase only — the gateway code is a placeholder for the email-delivered code officers would receive in production.
 * Officer IDs are generated deterministically from the role id.
 */
export const DEMO_GATEWAY_CODE = GATEWAY_CODE_BY_ROLE.agency;
export const DEMO_CREDENTIALS = ROLES.map((r, i) => ({
  role: r.id,
  title: r.title,
  officerId: `OFC-2026-${String(118 + i).padStart(4, "0")}`,
  pin: GATEWAY_CODE_BY_ROLE[r.id] ?? DEMO_GATEWAY_CODE,
}));
