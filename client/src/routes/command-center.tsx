...
function CommandCenter() {
  ...
  const [health, setHealth] = useState<Health[]>(() =>
    MINISTRIES.map((m) => ({
      ...
      rps: 10 + Math.random() * 80,
      uptime: 99 + Math.random(),
      status: "ok",
    })),
  );
  const [alerts, setAlerts] = useState<
    { id: number; sev: "high" | "med" | "low"; text: string; t: string }[]
  >([
    {
      id: 1,
      sev: "high",
      t: "12:41:08",
      text: "Anomaly: 4× NIN lookups from URA-CLI-781 in 9s · auto-throttled",
    },
    {
      id: 2,
      sev: "med",
      t: "12:39:51",
      text: "MoH adapter latency +28% above baseline · investigating",
    },
    ...
    const i = setInterval(() => {
      ...
      if (Math.random() > 0.7) {
        const samples = [
          {
            sev: "high" as const,
            text: "Suspicious cross-ministry probe blocked at gateway (zero-trust policy)",
          },
          {
            sev: "med" as const,
            text: "Spike: URA verification queue depth 412 → scaling adapter +1 replica",
          },
          {
            sev: "low" as const,
            text: "Audit ledger sealed batch · hash 0x" + Math.random().toString(16).slice(2, 10),
          },
          {
            sev: "high" as const,
            text: "AI flagged duplicate NIN claim · escalated to NIRA fraud desk",
          },
        ];
        const pick = samples[Math.floor(Math.random() * samples.length)];
        setAlerts((a) =>
          [{ id: Date.now(), t: new Date().toLocaleTimeString().slice(0, 8), ...pick }, ...a].slice(
            0,
            12,
          ),
        );
      ...
      <div className="max-w-7xl mx-auto px-4 py-8">
        <div className="flex flex-wrap items-end justify-between gap-3 mb-6">
          <div>
            ...
            <p className="mt-2 text-muted-foreground max-w-2xl text-sm">
              Real-time visibility into every ministry adapter, API gateway and security event
              across the NGDXH mesh.
            </p>
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 mb-6">
          {[
            ...
            {
              l: "Open incidents",
              v: alerts.filter((a) => a.sev === "high").length.toString(),
              i: AlertTriangle,
            },
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="glass rounded-xl p-5 lg:col-span-2">
            <div className="flex items-center justify-between mb-4">
              <div className="font-semibold flex items-center gap-2">
                <Activity className="w-4 h-4 text-gold" /> Adapter health
              </div>
                ...
                return (
                  <div key={m.id} className="border border-border/60 rounded-lg p-3 bg-card/40">
                    <div className="flex items-center gap-2">
                      <div
                        className="w-8 h-8 rounded font-mono text-[10px] font-bold flex items-center justify-center"
                        style={{
                      ...
                      <span
                        className={`text-[10px] px-1.5 py-0.5 rounded border ${ok ? "bg-success/15 text-success border-success/40" : "bg-warning/15 text-warning border-warning/40"}`}
                      >
                        {ok ? "HEALTHY" : "DEGRADED"}
                      </span>
                    </div>
                    <div className="mt-3 grid grid-cols-3 gap-2 text-[11px] font-mono">
                      <div>
                        ...
                        <div className="text-foreground">{Math.round(h.latency)}ms</div>
                      </div>
                      <div>
                        <div className="text-muted-foreground">rps</div>
                        <div className="text-foreground">{h.rps.toFixed(1)}</div>
                      </div>
                      <div>
                        <div className="text-muted-foreground">uptime</div>
                        <div className="text-success">{h.uptime.toFixed(2)}%</div>
                      </div>
                    </div>
                    <div className="mt-2 h-1.5 bg-muted rounded overflow-hidden">
                      <div
                        className="h-full bg-gradient-to-r from-success to-gold"
                        style={{ width: `${Math.min(100, h.rps * 1.2)}%` }}
                      />
                    </div>
                  </div>
                );
              ...
              {alerts.map((a) => (
                <div key={a.id} className="border border-border/60 rounded-md p-2.5 bg-card/40">
                  <div className="flex items-center gap-2 text-[10px] mb-1">
                    <span
                      className={`px-1.5 py-0.5 rounded font-mono ${a.sev === "high" ? "bg-destructive/15 text-destructive" : a.sev === "med" ? "bg-warning/15 text-warning" : "bg-success/15 text-success"}`}
                    >
                      {a.sev.toUpperCase()}
                    </span>
          ...
          <div className="glass rounded-xl p-5 lg:col-span-3">
            <div className="flex items-center justify-between mb-4">
              <div className="font-semibold flex items-center gap-2">
                <Globe2 className="w-4 h-4 text-gold" /> SLA matrix · last 30 days
              </div>
              <div className="text-xs text-muted-foreground flex gap-3">
                <span>
                  <span className="inline-block w-2 h-2 bg-success rounded mr-1" />
                  OK
                </span>
                <span>
                  <span className="inline-block w-2 h-2 bg-warning rounded mr-1" />
                  Degraded
                </span>
                <span>
                  <span className="inline-block w-2 h-2 bg-destructive rounded mr-1" />
                  Outage
                </span>
              </div>
            </div>
                ...
                <thead className="text-muted-foreground">
                  <tr>
                    ...
                    {Array.from({ length: 30 }).map((_, i) => (
                      <th key={i} className="font-mono text-[9px] w-4">
                        {i + 1}
                      </th>
                    ))}
                      ...
                      <tr key={m.id} className="border-t border-border/40">
                        ...
                        {cells.map((s, i) => (
                          <td key={i} className="px-0.5">
                            <div
                              className={`h-4 rounded-sm ${s === "ok" ? "bg-success/70" : s === "deg" ? "bg-warning/70" : "bg-destructive/70"}`}
                            />
                          </td>
                        ))}
                        <td className="px-2 text-right text-success font-mono">
                          {((ok / 30) * 100).toFixed(1)}%
                        </td>
                      </tr>
                    );
            ...
            <div className="mt-4 flex items-center gap-2 text-xs text-muted-foreground">
              <CheckCircle2 className="w-4 h-4 text-success" /> All adapters within national SLA
              tolerance (99.9% target).
            </div>
          </div>
        </div>
      </div>
