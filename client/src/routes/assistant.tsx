...
import { Bot, Languages, Mic, Send, Sparkles, User2 } from "lucide-react";
...
function Assistant() {
  ...
  const send = (q: string) => {
    ...
    setTimeout(
      () => {
        setMessages((current) => [...current, { role: "bot", text: respond(q) }]);
        setTyping(false);
      },
      700 + Math.random() * 600,
    );
      ...
      <div className="max-w-5xl mx-auto px-4 py-10">
        <div className="mb-6">
          <div className="text-xs uppercase tracking-[0.2em] text-gold mb-2 flex items-center gap-2">
            ...
            AI citizen assistant · multilingual
          </div>
        ...
        <div className="glass rounded-xl flex flex-col h-160 overflow-hidden">
          <div className="px-5 py-3 border-b border-border flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-linear-to-br from-primary to-gold flex items-center justify-center text-primary-foreground">
            ...
            <div className="flex-1">
              ...
              <div className="text-[11px] text-muted-foreground flex items-center gap-1">
                ...
                Online grounded in 9 ministry APIs
              </div>
            </div>
            <button
              type="button"
              aria-label="Change assistant language"
              className="text-xs flex items-center gap-1 px-2 py-1 rounded border border-border hover:bg-accent"
            >
          ...
          <div ref={scrollRef} className="flex-1 overflow-auto p-5 space-y-4">
              ...
              <div key={i} className={`flex gap-3 ${m.role === "user" ? "flex-row-reverse" : ""}`}>
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${
                    m.role === "user"
                      ? "bg-accent"
                      : "bg-linear-to-br from-primary to-gold text-primary-foreground"
                  }`}
                >
            ...
            {typing ? (
              <div className="flex gap-3">
                <div className="w-8 h-8 rounded-full bg-linear-to-br from-primary to-gold flex items-center justify-center">
