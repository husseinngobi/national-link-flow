...
const STEPS = [
  { k: "capturing", label: "Capturing biometric frame", detail: "640×640 RGB · WebRTC stream simulated" },
  { k: "extracting", label: "Extracting 512-d face embedding", detail: "ArcFace · ONNX runtime · GPU" },
  { k: "matching", label: "Searching National Biometric Index", detail: "9.4M templates · FAISS · k=5 neighbours" },
  { k: "done", label: "Match resolved · dossier assembled", detail: "Cross-ministry profile via NGDXH" },
];
    ...
    <InternalShell>
      <div className="max-w-7xl mx-auto px-4 py-10">
        <div className="mb-8">
          ...
          <h1 className="text-3xl lg:text-4xl font-display font-bold">Identify a citizen from a face alone</h1>
          <p className="mt-2 text-muted-foreground max-w-2xl">
            When no ID is presented, NGDXH can match a captured face against the National Biometric Index
            (NIRA + UPF + DCIC) and assemble a full cross-ministry profile in seconds — a capability
            today's UG Hub does not offer.
          </p>
        </div>
        ...
        <div className="grid lg:grid-cols-[440px_1fr] gap-6">
            ...
            <div className="aspect-square rounded-lg overflow-hidden relative bg-[color:var(--color-surface-strong)] border border-border">
              ...
              <div className="absolute inset-[18%] border-2 border-gold rounded-md">
                ...
                {["top-0 left-0", "top-0 right-0", "bottom-0 left-0", "bottom-0 right-0"].map((p) => (
                  <span key={p} className={`absolute ${p} w-3 h-3 border-gold`} style={{ borderTopWidth: p.includes("top") ? 2 : 0, borderBottomWidth: p.includes("bottom") ? 2 : 0, borderLeftWidth: p.includes("left") ? 2 : 0, borderRightWidth: p.includes("right") ? 2 : 0 }} />
                ))}
              </div>
              ...
              <div className="absolute top-2 left-2 right-2 flex items-center justify-between text-[10px] font-mono">
                ...
                <span className="px-1.5 py-0.5 rounded bg-background/70 text-muted-foreground">CAM-01 · 1080p · 30fps</span>
              </div>
            </div>

            <div className="mt-4 flex gap-2">
              {phase === "idle" || phase === "done" ? (
                <button onClick={phase === "done" ? reset : run}
                  ...
                  <Camera className="w-4 h-4" /> {phase === "done" ? "Scan another" : "Start face scan"}
                </button>
              ) : (
                <button disabled className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-md bg-primary/70 text-primary-foreground font-medium">
          ...
          <div className="space-y-4">
            ...
            <div className="glass rounded-xl p-5">
              ...
              <div className="space-y-3">
                {STEPS.map((s, i) => {
                  ...
                  const state = phase === "idle" ? "idle" : me < cur ? "done" : me === cur ? "active" : "pending";
                  return (
                    <div key={s.k} className="flex items-start gap-3">
                      <div className={`mt-0.5 w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-mono ${
                        state === "done" ? "bg-success/20 text-success border border-success/40"
                        : state === "active" ? "bg-primary/20 text-primary border border-primary/40"
                        : "bg-accent text-muted-foreground border border-border"
                      }`}>
                        {state === "done" ? <CheckCircle2 className="w-3.5 h-3.5" /> : state === "active" ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : i + 1}
                      </div>
                ...
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="glass rounded-xl p-5">
                  ...
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-muted-foreground font-mono">{c.nin} · DOB {c.dob} · {c.sex}</div>
                    </div>
                  </div>
                  ...
                  <div className="grid sm:grid-cols-3 gap-2 text-xs">
                    {[
                      ...
                      ["Criminal", c.criminal.length === 0 ? "Clear" : `${c.criminal.length} case(s)`],
                    ].map(([k, v]) => (
                  ...
                  <Link to="/verify" className="mt-4 inline-flex items-center gap-2 text-sm text-primary hover:underline">
            ...
            <div className="glass rounded-xl p-5 grid sm:grid-cols-3 gap-3 text-xs">
              {[
                { i: <Cpu className="w-4 h-4 text-gold" />, t: "ArcFace embeddings", d: "512-dimension vectors · cosine match" },
                { i: <Shield className="w-4 h-4 text-gold" />, t: "Liveness detection", d: "Anti-spoof: blink, depth, IR plane" },
                { i: <AlertTriangle className="w-4 h-4 text-gold" />, t: "Watchlist alerts", d: "Interpol & UPF flags surfaced live" },
              ].map((b) => (
