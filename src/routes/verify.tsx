...
import {
  ...
  Car, Home as HomeIcon, Receipt, GraduationCap, Briefcase, Plane, Smartphone, Gavel, HeartPulse, Users as UsersIcon,
} from "lucide-react";
    ...
    <InternalShell>
      <div className="max-w-7xl mx-auto px-4 py-10">
        <div className="mb-8">
          ...
          <p className="mt-2 text-muted-foreground max-w-2xl">Enter a National Identification Number. NGDXH will fetch authorized verification responses from each connected ministry in real time and assemble a complete profile.</p>
        </div>
          ...
          <div className="space-y-3">
              ...
              {shown && (
                <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} className="glass rounded-xl p-5">
                  <div className="flex items-center gap-4 flex-wrap">
                    <div className="w-16 h-16 rounded-full bg-gradient-to-br from-primary to-gold flex items-center justify-center text-xl font-bold text-primary-foreground">{c.photoInitial}</div>
                    <div className="flex-1 min-w-[200px]">
                      ...
                      <div className="text-sm text-muted-foreground font-mono">{c.nin} · DOB {c.dob} · {c.sex} · {c.district}</div>
                    </div>
                  ...
                  <div className="mt-5 grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
                    {[
                      ["Tribe", c.tribe], ["Religion", c.religion], ["Marital", c.maritalStatus],
                      ["Dependants", String(c.dependants)], ["Phone", c.phone], ["Email", c.email],
                      ["Parish", c.parish], ["Village", c.village],
                    ].map(([k, v]) => (
                      <div key={k} className="glass rounded-md p-2.5">
                        <div className="uppercase tracking-wider text-[10px] text-muted-foreground">{k}</div>
                  ...
                  <div className="mt-4 glass rounded-md p-3 text-xs">
                    <div className="flex items-center gap-2 mb-1.5 text-gold uppercase tracking-wider text-[10px]"><UsersIcon className="w-3 h-3" /> Family graph</div>
                    <div className="grid sm:grid-cols-3 gap-2">
                      <div><span className="text-muted-foreground">Father · </span>{c.father}</div>
                      <div><span className="text-muted-foreground">Mother · </span>{c.mother}</div>
                      <div><span className="text-muted-foreground">Spouse · </span>{c.spouse}</div>
                    </div>
                  </div>
                </motion.div>
              )}
            ...
            {shown && (
              <div className="grid md:grid-cols-2 gap-3">
                <DossierCard icon={<Car className="w-4 h-4 text-gold" />} title="Vehicles & permits (MoWT)">
                  {c.vehicles.map((v) => (
                    <Row key={v.plate} l={v.plate} r={`${v.make} · ${v.color}`} sub={v.status} />
                  ))}
                </DossierCard>

                <DossierCard icon={<HomeIcon className="w-4 h-4 text-gold" />} title="Land & property (MLHUD)">
                  {c.properties.map((p) => (
                    <Row key={p.title} l={p.title} r={p.location} sub={`${p.type} · ${p.size}`} />
                  ))}
                </DossierCard>

                <DossierCard icon={<Receipt className="w-4 h-4 text-gold" />} title="Tax history (URA)">
                  {c.taxHistory.map((t) => (
                    <Row key={t.year} l={t.year} r={`${t.paye} PAYE · ${t.vat} VAT`} sub={t.status} />
                  ))}
                </DossierCard>

                <DossierCard icon={<Gavel className="w-4 h-4 text-gold" />} title="Criminal record (UPF · Judiciary)">
                  {c.criminal.length === 0 ? (
                    <div className="text-xs text-success flex items-center gap-1.5">
                      <CheckCircle2 className="w-3.5 h-3.5" /> No criminal cases on file
                    </div>
                  ) : (
                    c.criminal.map((cr) => <Row key={cr.caseNo} l={cr.caseNo} r={cr.offence} sub={`${cr.status} · ${cr.date}`} />)
                  )}
                </DossierCard>

                <DossierCard icon={<GraduationCap className="w-4 h-4 text-gold" />} title="Education (MoES)">
                  {c.education.map((e) => <Row key={e.award} l={e.award} r={e.institution} sub={e.year} />)}
                </DossierCard>

                <DossierCard icon={<Briefcase className="w-4 h-4 text-gold" />} title="Employment">
                  {c.employment.map((e) => <Row key={e.employer} l={e.role} r={e.employer} sub={`Since ${e.since} · ${e.status}`} />)}
                </DossierCard>

                <DossierCard icon={<Smartphone className="w-4 h-4 text-gold" />} title="Telecom (UCC)">
                  {c.telecom.map((t) => <Row key={t.msisdn} l={t.msisdn} r={`${t.network} · KYC ${t.kyc}`} sub={`Registered ${t.since}`} />)}
                </DossierCard>

                <DossierCard icon={<Plane className="w-4 h-4 text-gold" />} title="Travel history (DCIC)">
                  {c.travel.map((t) => <Row key={t.date} l={t.date} r={t.port} sub={`${t.carrier} · ${t.purpose}`} />)}
                </DossierCard>

                <DossierCard icon={<HeartPulse className="w-4 h-4 text-gold" />} title="Health (MoH · restricted)">
                  {c.health.map((h) => (
...
function DossierCard({ icon, title, children }: { icon: React.ReactNode; title: string; children: React.ReactNode }) {
  return (
