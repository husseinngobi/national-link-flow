...
import {
  Car, Plane, Receipt, HeartPulse, GraduationCap, Building2, IdCard,
  FileText, Phone, Briefcase, Baby, Scale, ArrowRight,
} from "lucide-react";
...
export const Route = createFileRoute("/services")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Browse and access government services from every Ugandan ministry in one secure place." },
    ],
  }),
});
...
const CATEGORIES = [
  {
    ...
    icon: IdCard,
    items: ["National ID (NIN) application", "Birth certificate", "Marriage certificate", "Death certificate"],
  },
  {
    ...
    <SiteShell>
      <div className="max-w-7xl mx-auto px-4 py-12">
        <div className="mb-10 max-w-2xl">
          <div className="text-xs uppercase tracking-[0.2em] text-gold mb-2">Government services</div>
          <h1 className="text-3xl lg:text-4xl font-display font-bold">Every service. One trusted place.</h1>
          ...
          {CATEGORIES.map((c) => (
            <div key={c.title} className="glass rounded-xl p-5 hover:border-gold/40 transition">
              ...
              <ul className="space-y-1.5 text-sm">
                {c.items.map((item) => (
                  <li key={item} className="flex items-center gap-2 text-muted-foreground hover:text-foreground transition">
        ...
        <div className="mt-12 glass rounded-xl p-6 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <div className="text-sm text-muted-foreground">Ask our AI assistant or call our toll-free line.</div>
            </div>
          </div>
          <div className="flex gap-2">
            <Link to="/assistant" className="px-4 py-2 rounded-md bg-primary text-primary-foreground text-sm font-medium hover:bg-primary/90 inline-flex items-center gap-1.5">
              Chat with AI <ArrowRight className="w-3.5 h-3.5" />
            </Link>
            <a href="tel:0800100000" className="px-4 py-2 rounded-md border border-border text-sm hover:bg-accent inline-flex items-center gap-1.5">
