...
const REASONS = ["Citizen verification", "Cross-agency lookup", "Compliance audit", "Fraud investigation", "KYC onboarding", "Service eligibility"];
const ACTIONS = ["identity.read", "tax.read", "health.insurance.read", "criminal.read", "land.read", "permit.read", "passport.read"];
...
function AuditPage() {
  ...
  const filtered = logs.filter((l) =>
    ...
    (q === "" || l.target.toLowerCase().includes(q.toLowerCase()) || l.officer.toLowerCase().includes(q.toLowerCase()) || l.action.includes(q))
  );
    ...
    <InternalShell>
      <div className="max-w-7xl mx-auto px-4 py-10">
        <div className="mb-8">
          ...
          <h1 className="text-3xl lg:text-4xl font-display font-bold">Every request. Every reason. Forever.</h1>
          <p className="mt-2 text-muted-foreground max-w-3xl text-sm">
            Each cross-agency exchange is hashed, signed and sealed in an append-only ledger. The Auditor General — and the citizen themselves — can verify who accessed what data, when, and why.
          </p>
        </div>
        ...
        <div className="glass rounded-xl p-5">
          <div className="flex items-center gap-2 flex-wrap mb-4">
            <div className="font-semibold flex items-center gap-2 mr-auto"><FileText className="w-4 h-4 text-gold" /> Immutable transaction ledger</div>
            <div className="relative">
              <Search className="w-3.5 h-3.5 text-muted-foreground absolute left-2 top-1/2 -translate-y-1/2" />
              <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search NIN, officer, action…" className="bg-input border border-border rounded-md pl-7 pr-2 py-1.5 text-xs w-64" />
            </div>
            <div className="flex items-center gap-1">
              <Filter className="w-3.5 h-3.5 text-muted-foreground" />
              <select value={ministry} onChange={(e) => setMinistry(e.target.value)} className="bg-input border border-border rounded-md px-2 py-1.5 text-xs">
                <option value="all">All ministries</option>
                {MINISTRIES.map((m) => <option key={m.id} value={m.code}>{m.code}</option>)}
              </select>
            </div>
          </div>
            ...
            <table className="w-full text-xs">
              <thead className="text-muted-foreground">
                <tr className="border-b border-border/60">
                  ...
                  <th className="text-left font-medium px-2 py-2"><User className="w-3 h-3 inline -mt-0.5" /> Officer</th>
                  ...
                  <th className="text-right font-medium px-2 py-2"><Hash className="w-3 h-3 inline -mt-0.5" /> Block</th>
                </tr>
              </thead>
                ...
                {filtered.map((l) => (
                  <tr key={l.id} className="border-b border-border/30 hover:bg-accent/30">
                    ...
                    <td className="px-2 py-1.5 text-right">
                      <span className={`px-1.5 py-0.5 rounded ${l.result === "allowed" ? "bg-success/15 text-success" : "bg-destructive/15 text-destructive"}`}>{l.result}</span>
                    </td>
          ...
          <div className="mt-4 flex items-center justify-between text-xs text-muted-foreground">
            <span>{filtered.length} of {logs.length} records · live tail enabled</span>
            <span className="font-mono">Ledger root: 0xc4f1…a87b · sealed by Auditor General PKI</span>
          </div>
        </div>
      </div>
    </InternalShell>
