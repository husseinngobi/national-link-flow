...
function Assistant() {
  const [messages, setMessages] = useState<Msg[]>([
    { role: "bot", text: "Hello! I'm the **NGDXH AI Citizen Assistant** — your guide to Ugandan government services. Ask me anything in English, Luganda, Runyankole, Acholi or Lusoga." },
  ]);
  ...
  return (
    <SiteShell>
      <div className="max-w-5xl mx-auto px-4 py-10">
        <div className="mb-6">
          <div className="text-xs uppercase tracking-[0.2em] text-gold mb-2 flex items-center gap-2"><Sparkles className="w-3.5 h-3.5" /> AI citizen assistant · multilingual</div>
          <h1 className="text-3xl lg:text-4xl font-display font-bold">Ask. Understand. Get served.</h1>
          <p className="mt-2 text-muted-foreground max-w-2xl text-sm">
            A conversational guide to every Ugandan government service — grounded in real ministry data through NGDXH. No more queues, no more confusion.
          </p>
        </div>
        ...
        <div className="glass rounded-xl flex flex-col h-[640px] overflow-hidden">
          ...
          <div className="px-5 py-3 border-b border-border flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-primary to-gold flex items-center justify-center text-primary-foreground"><Bot className="w-5 h-5" /></div>
            <div className="flex-1">
              <div className="font-semibold text-sm">NGDXH Assistant</div>
              <div className="text-[11px] text-muted-foreground flex items-center gap-1"><span className="w-1.5 h-1.5 bg-success rounded-full animate-pulse" /> Online · grounded in 9 ministry APIs</div>
            </div>
            <button className="text-xs flex items-center gap-1 px-2 py-1 rounded border border-border hover:bg-accent"><Languages className="w-3.5 h-3.5" /> EN</button>
          </div>
          ...
          <div ref={scrollRef} className="flex-1 overflow-auto p-5 space-y-4">
            {messages.map((m, i) => (
              <div key={i} className={`flex gap-3 ${m.role === "user" ? "flex-row-reverse" : ""}`}>
                <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 ${m.role === "user" ? "bg-accent" : "bg-gradient-to-br from-primary to-gold text-primary-foreground"}`}>
                  {m.role === "user" ? <User2 className="w-4 h-4" /> : <Bot className="w-4 h-4" />}
                </div>
                <div className={`max-w-[78%] rounded-2xl px-4 py-2.5 text-sm leading-relaxed whitespace-pre-wrap ${m.role === "user" ? "bg-primary text-primary-foreground rounded-tr-sm" : "bg-card border border-border rounded-tl-sm"}`}>
                  {m.text.split("**").map((seg, j) => j % 2 === 1 ? <strong key={j} className="text-gold">{seg}</strong> : <span key={j}>{seg}</span>)}
                </div>
              </div>
            ))}
            {typing && (
              <div className="flex gap-3">
                <div className="w-8 h-8 rounded-full bg-gradient-to-br from-primary to-gold flex items-center justify-center"><Bot className="w-4 h-4 text-primary-foreground" /></div>
          ...
          {messages.length <= 1 && (
            <div className="px-5 pb-2 flex flex-wrap gap-2">
              {SUGGESTIONS.map((s) => (
                <button key={s} onClick={() => send(s)} className="text-xs px-3 py-1.5 rounded-full border border-border hover:bg-accent">{s}</button>
              ))}
            </div>
          )}
          ...
          <div className="border-t border-border p-3 flex items-center gap-2">
            <button className="p-2 rounded-md hover:bg-accent text-muted-foreground"><Mic className="w-4 h-4" /></button>
            <input
              value={input}
            ...
            />
            <button onClick={() => send(input)} className="px-3 py-2 rounded-md bg-primary text-primary-foreground hover:bg-primary/90 inline-flex items-center gap-1.5 text-sm font-medium"><Send className="w-4 h-4" /> Send</button>
          </div>
        </div>

        <div className="mt-4 text-xs text-muted-foreground text-center">
          Prototype assistant · in production, NGDXH grounds responses against live ministry knowledge bases under the Data Protection &amp; Privacy Act 2019.
        </div>
      </div>
    </SiteShell>
  );
